    ...
    methods: {
      ...
      async open() {
        ...
        this.$alert(`<img src=${url} />`, '请用微信支付', {
          ...
          //一些布局的修改
          center:true,showCancelButton:true,cancelButtonText:'支付遇见问题',confirmButtonText:'已支付成功',showClose:false,
          beforeClose:(type,instance,done)=>{
            //type区分取消/确定按钮。instance当前组件实例，done关闭弹出框的方法
            if(type=='cancel'){
              alert('请联系管理员只只');
              //清除定时器
        ...
        if(!this.timer){
            this.timer = setInterval(async() => {
              let r = await this.$API.reqPayStatus(this.orderId);
              //支付成功200，支付中205
              if(r.code==200){
                //清除定时器
                clearInterval(this.timer);
